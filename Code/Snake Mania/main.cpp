    ...
    int update(int x, int y)
    {
        if(x>67||x<10||y>22||y<3)				//for checking if snake hit the wall
            GameOver();						  // Gameover() function called								
                ...
                createRand();                    // creation of food
        gotoxy(lastx, lasty);				// algorithm for increase in the size of the snake
        printf("o");
        gotoxy(x, y);
        nodex=x;     //this is for the body of the snake after the first initialization of 
        nodey=y;     //of the body of the snake
        posx=x;      //co-ordinate for the mouth of the  
        posy=y;      //the snake
        printf("@");
        lastx=x;  // update the coordiante of the tail of the snake 
        lasty=y;
    }
    void CreateNode(int x, int y, int i)				  //body of the snake
    {
        place=i;			// i is changing with the for loop 
        nodex=x;
        nodey=y;
        gotoxy(nodex, nodey);
        printf("%c", 489);
...
class snake
{
    ...
    void move_right()
    {
        if(i==sizes)			 // i is changing with the for loop when i== sizes happens that for loop again start working
        i=0;					 // as we again assigned i=0
